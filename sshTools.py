...
def Info_(d_node):
    ...
    if 'Arista' in Vendor_List:
        print ('-' * 110)
        print ("This is a %s %s Device for %s" %(d_node.manu,d_node.device_type,d_node.project))
        print ("SN: %s | App_Name: %s | Model: %s | Customer: %s" %(d_node.serial,d_node.app_name,d_node.model,d_node.customer))
        print ("ip: %s | Login: %s | PW: %s" %(d_node.node_ip,str(d_node.user),str(d_node.pw)))
...
if __name__ == '__main__':
    ...
    d_node = Device(node_id,serial,manu,model,customer,device_type,app_name,node_ip,project)
