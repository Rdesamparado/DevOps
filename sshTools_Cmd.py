import paramiko
import sys
from sshTools_Class import ColorTerminalOutput, Servicenowinfo, Robloxinfo, Cumulusinfo, Avocentinfo
import time
...
def pduportassign(d_node):
    ...
    if 'PDU-B1' in Device_Fnder:
        ...
        print(cmd)
        print('PDU Port Assign started for PDU-B1')
...
def connect_(d_node, action):
    ...
    if action == 'usage':
        ...
        Snowmodel = ('Raritan', 'Gelu')

        if str(Dvice) not in Snowmodel:
            print('\n')
            print(
                "\033[91m     Action selected is not available for this Device.\033[0m")
            print('\n')
            choice(d_node)

        if action == '2' and str(Dvice) in Snowmodel:
            print('')
            ...
            time.sleep(2)
            channel.send(
                'network ipv4 interface ETH1 preferredHostName PDU-%s\n' % (serial))
        ...
        elif action == '3' and str(Dvice) == "Raritan":
            print('')
        ...
        elif action == '3' and str(Dvice) == "Gelu":
            print('')
            print('Collecting Servicenow\'s Gelu PDU Logs')
            print('-'*110)
            time.sleep(3)
            channel.send('show pdu\n')
            time.sleep(2)
            channel.send('show network\n')
            time.sleep(2)
            channel.send('show outlets\n')
        ...
        if 'Servertech' not in Dvice:
            print('\n')
            print(
                "\033[91m     Action selected is unavailable for this Device.\033[0m ")
            print('\n')
            choice(d_node)
            ...
            try:
                if str(Resp) == 'yes' or str(Resp) == 'y':
                    ...
                    time.sleep(2)
                    restart = input(
                        "Type \"Yes\" to restart device to apply changes: ")
        ...
        if '210-AEDQ' not in Dmodel:
            ...
            time.sleep(3)
            channel.send(
                'fabien.broquet@servicenow.com|4CtjMCACDi5ctakKPHDBMa+EfmOVAg4IIO33JXhBpDZSERL11Q\n')
            time.sleep(1)
            ...
            time.sleep(5)
            channel.send(
                'sudo ethtool -m swp49 | grep -E "Vendor PN|Vendor SN"\n')
            time.sleep(1)
            channel.send(node_pw_1 + ('\n'))
            time.sleep(1)
            channel.send(
                'sudo ethtool -m swp50 | grep -E "Vendor PN|Vendor SN"\n')
            time.sleep(1)
            channel.send(
                'sudo ethtool -m swp51 | grep -E "Vendor PN|Vendor SN"\n')
            time.sleep(1)
            channel.send(
                'sudo ethtool -m swp52 | grep -E "Vendor PN|Vendor SN"\n')
            time.sleep(1)
            output = channel.recv(99999).decode('utf-8')
        ...
        if action == '11' and 'Avocent' in d_node.manu:
            ...
            channel.send(
                'set users/local_accounts/user_names/root/settings/ status=enabled\n')
            time.sleep(1)
            channel.send(
                'set users/local_accounts/user_names/root/settings/ password=P@ssw0rd confirm_password=P@ssw0rd\n')
            time.sleep(1)
            channel.send(
                'set users/local_accounts/user_names/root/settings/ password_change_at_next_login=no\n')
            time.sleep(1)
            channel.send('commit\n')
            time.sleep(3)
            output = channel.recv(99999).decode('utf-8')
            ...
            time.sleep(1)
            channel.send(
                'show users/local_accounts/user_names/root/settings/\n')
            time.sleep(2)
            output = channel.recv(99999).decode('utf-8')
            print(output)
            choice(d_node)
            ...
            print('')
            confirm = input(
                "Type 'yes' to continue restarting device to Factory Default: ").lower()
